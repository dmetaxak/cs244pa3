...
sudo xterm -e $SPROUT_BT2 $qdisc $egress $client_mac &

# shuttle packets between tap-client and ingress_interface
sudo xterm -e $SPROUT_BT2 10.0.0.1 60001 $qdisc $ingress $client_mac &

# Setup cellsim between them
# Get tap-clients MAC address
tap_client_mac=`ifconfig tap-client | grep HWaddr | awk '{print $5}'`

sudo xterm -e ~/cs244pa3/multisend/sender/cellsim-tap $downlink $uplink $tap_client_mac -1 tap-relay tap-client &
