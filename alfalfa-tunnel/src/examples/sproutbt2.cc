...
int main( int argc, char *argv[] )
{
  ...
  if ( argc == 6 ) {
    ...
    client_mac = argv[ 3 ];
    fprintf( stderr, "getting network connection");
    net = new Network::SproutConnection( NULL, NULL );
    fprintf( stderr, "network connection is set up");
  } else {
    fprintf( stderr, "Invalid number of arguments \n" );
    fprintf( stderr, "Usage (server): ./sproutbt2 qdisc interface_name client_mac");
    fprintf( stderr, "Usage (client): ./sproutbt2 ip port qdisc interface_name client_mac");
    fprintf( stderr, "qdisc: [CoDel | Sprout], interface_name: e.g. eth0");
    exit(-1);
  }

  PacketSocket eth_socket ;
  if (server) {
    fprintf( stderr, "setting up ethernet socket");
